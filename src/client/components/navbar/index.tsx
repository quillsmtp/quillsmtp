	...
	return (
		<>
			<div className="qsmtp-header">
				<div className="qsmtp-logo"><img width="40" src={qsmtpAdmin?.assetsBuildUrl + "assets/logo.svg"} alt="logo" />
					<h2>Quill SMTP</h2>
				</div>
				<NavLink className="qsmtp-header-link" to="/admin.php?page=quillsmtp" exact isActive={(match, location) => {
					if (
						location.pathname ===
						"" ||
						location.pathname ===
				...
				<NavLink className="qsmtp-header-link" to={`/admin.php?page=quillsmtp&path=settings`} exact isActive={(match, location) => {
					if (
						location.pathname ===
						"settings"
					) {
				...
				<NavLink className="qsmtp-header-link" to="/admin.php?page=quillsmtp&path=email-test" exact isActive={(match, location) => {
					if (
						location.pathname ===
						"email-test"
					) {
				...
				<NavLink to="/admin.php?page=quillsmtp&path=logs" className="qsmtp-header-link" exact isActive={(match, location) => {
					if (
						location.pathname ===
						"logs"
					) {
				...
				<NavLink to="/admin.php?page=quillsmtp&path=debug" className="qsmtp-header-link" exact isActive={(match, location) => {
					if (
						location.pathname ===
						"debug"
					) {
