 ...
 */
namespace QuillSMTP\Vendor\Monolog;

use Closure;
use DateTimeZone;
use Fiber;
use QuillSMTP\Vendor\Monolog\Handler\HandlerInterface;
use QuillSMTP\Vendor\Monolog\Processor\ProcessorInterface;
use QuillSMTP\Vendor\Psr\Log\LoggerInterface;
use QuillSMTP\Vendor\Psr\Log\InvalidArgumentException;
...
class Logger implements LoggerInterface, ResettableInterface
{
    ...
    public function addRecord(int|Level $level, string $message, array $context = [], DateTimeImmutable|null $datetime = null) : bool
    {
        ...
        try {
            ...
            $record = new LogRecord(datetime: $datetime ?? new DateTimeImmutable($this->microsecondTimestamps, $this->timezone), channel: $this->name, level: self::toMonologLevel($level), message: $message, context: $context, extra: []);
            $handled = \false;
